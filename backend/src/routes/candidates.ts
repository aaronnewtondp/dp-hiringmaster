...
// ─── GET /api/candidates/unmatched ──────────────────────────────────────────
// Must be registered before GET /:id (Express would otherwise match this path
// as an :id value). "Unmatched Candidates" (merged 2026-09-05 — used to be
// two separate banners on Candidates.tsx, "Unlinked candidates" and
// "Unmatched role submissions"; the latter is a strict subset of the former
// by construction, since a Job Application Form no-match never gets an
// application at all, so folding them into one list + one format is a real
// simplification, not a data change): every candidate with zero applications,
// whether or not they came through a Job Application Form submission whose
// role text never matched a role. A row from the form-submission half still
// carries its submitted_text/suggested_role_* (for the one-click "suggested
// role" quick-resolve); a plain manually-added-with-no-application candidate
// carries those as null. See utils/unmatchedCandidates.ts — also the source
// for dashboard.ts's candidates_unmatched KPI, so the same term never means
// two different counts on two different pages.
router.get('/unmatched', requireHR, async (req: Request, res: Response) => {
  const { limit = '50', offset = '0' } = req.query;
  const limitNum = parseInt(limit as string);
  const offsetNum = parseInt(offset as string);

  const { rows, total } = await fetchUnmatchedCandidates(limitNum, offsetNum);

  res.json({ submissions: rows, total, limit: limitNum, offset: offsetNum });
});
