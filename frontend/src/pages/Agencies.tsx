                ...
                <tr key={ag.id} className={`hover:bg-gray-50 cursor-pointer ${idx % 2 ? 'bg-gray-50/40' : ''}`}>
                  <td className="table-td px-2">
                    <Link to={`/agencies/${ag.id}`} className="block">
                      <div className="font-medium text-gray-900 hover:text-dp-600 truncate">{ag.name}</div>
                      {ag.contact_name && <div className="text-xs text-gray-400 truncate">{ag.contact_name}</div>}
                    </Link>
                  </td>
                  <td className="table-td px-2">
                  ...
                  <td className="table-td px-2 text-xs text-gray-600 line-clamp-2">
                    {ag.specialisations || <span className="text-gray-300">—</span>}
                  </td>
                  <td className="table-td px-2 text-xs font-mono text-gray-600">
                    {ag.tier1_rate && <div className="truncate">{ag.tier1_band || 'All'}: {ag.tier1_rate}</div>}
                    {ag.tier2_rate && <div className="truncate">{ag.tier2_band}: {ag.tier2_rate}</div>}
                    {ag.tier3_rate && <div className="truncate">{ag.tier3_band}: {ag.tier3_rate}</div>}
                  ...
                  <td className="table-td px-2 text-center text-green-700 font-medium font-mono">{ag.total_hired ?? 0}</td>
                  <td className="table-td px-2 text-xs text-gray-500">
                    <div className="font-mono">{ag.replacement_guarantee_days ?? 60}d window</div>
                    {ag.replacement_triggers && <div className="text-gray-400 mt-0.5 line-clamp-2">{ag.replacement_triggers}</div>}
                  </td>
                  <td className="table-td px-2 text-xs text-gray-500">
                    {ag.billing_effective_window
                      ? <div className="font-mono text-gray-600 truncate">{ag.billing_effective_window}</div>
                      : <span className="text-gray-300">—</span>}
                    {ag.billing_late_penalty && ag.billing_late_penalty !== 'None stated' && (
                      <div className="text-amber-600 mt-0.5 line-clamp-2">{ag.billing_late_penalty}</div>
                    )}
                  </td>
                  <td className="table-td px-2">
